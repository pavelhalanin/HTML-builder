  ...
  static logScore() {
    console.log(
      [
        ...
        'Task 06 Build Page',
        'EN',
        "- [x] 0 / 20 After running 'node 06-build-page', the project-dist folder is created and contains index.html, style.css, and an `assets/` folder",
        '- [x] 0 / 35 index.html is built by substituting every {{component-name}} tag in template.html with the contents of `components/<component-name>.html`',
        "- [x] 0 / 20 `style.css` is a bundle of all `.css` files from the '`styles`' folder",
        '- [x] 0 / 20 `assets/` is an exact copy of `06-build-page/assets/`',
        '- [x] 0 / 10 The original `template.html` is not modified by the script',
        "- [x] 0 / 10 Two template tags written on the same line separated only by spaces (e.g. '`{{about}} {{articles}}`') are processed as separate components without errors",
        '- [x] 0 / 20 Rerunning the script after a new component is added to `components/` and its tag is added to template.html correctly updates `project-dist/index.html`. Changes inside `styles/` and `assets/` are also picked up',
        'RU:',
        "- [x] 0 / 20 После запуска 'node 06-build-page' создаётся папка project-dist, содержащая index.html, style.css и папку `assets/`",
        '- [x] 0 / 35 index.html собирается путём замены каждого тега {{имя-компонента}} в template.html на содержимое `components/<имя-компонента>.html`',
        "- [x] 0 / 20 `style.css` является бандлом всех файлов `.css` из папки '`styles`'",
        '- [x] 0 / 20 `assets/` является точной копией `06-build-page/assets/`',
        '- [x] 0 / 10 Исходный `template.html` не изменяется скриптом',
        "- [x] 0 / 10 Два тега шаблона, записанных в одной строке с пробелами между ними (например, '`{{about}} {{articles}}`'), обрабатываются как отдельные компоненты без ошибок",
        '- [x] 0 / 20 Повторный запуск скрипта после добавления нового компонента в `components/` и добавления его тега в template.html корректно обновляет `project-dist/index.html`. Изменения внутри `styles/` и `assets/` также учитываются',
        '> > > > > > > >\n',
      ].join('\n'),
    );
  }
